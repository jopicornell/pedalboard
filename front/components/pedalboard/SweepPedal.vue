...
const percent = computed(() =>
  props.sweep?.value != null && props.sweep?.value > -1 ? (props.sweep.value - props.sweep.min) / (props.sweep.max - props.sweep.min) * 100 : null
)
...
const handleSweepClick = (event: PointerEvent) => {
  if (props.changeOnClick && props.sweep !== undefined && mouseDown.value) {
    const tolerance = 5
    if (sweepButton.value === null) return
    const height = sweepButton.value.offsetHeight - tolerance * 2
...
<template>
    ...
    >
        ...
        <div
v-if="sweep" class="bg-cyan-800 from-indigo-500 via-purple-500 to-pink-500 w-full rounded-lg px-5"
             :style="percentStyle">
