...
import { type Input, type Output, type MessageEvent } from 'webmidi'
...
import EditButtonModal from '@/components/modals/EditButtonModal.vue';
...
const editButtonModal = ref<InstanceType<typeof EditButtonModal> | null>(null)
...
const selectedBankButtons = computed<Button[]>((): Button[] => {
  ...
  if (hasDirectBank && selectedBankIndex.value > 0) {
    return [...selectedBank.value.buttons, { name: 'Go to Looper', position: 9, type: 'goToDirectBank' }]
  }
  return selectedBank.value.buttons
})
...
const handleButtonClick = (index: number) => {
  if (isEditing.value) {
    editButtonModal.value?.open(selectedBankButtons.value[index])
  } else {
...
const handleBankUp = () => {
  const hasDirectBank = banks.value[0].name === 'Direct Bank'
...
const handleButtonPressedFromPedal = (button: Button, event: MessageEvent) => {
  ...
  if (button.type === 'effect' && event.rawValue !== undefined && button.enabled !== event.rawValue > 0) {
    const foundButton = banks.value[selectedBankIndex.value].buttons.find(btn => btn.id === button?.id)
    ...
    foundButton.enabled = event.rawValue > 0
    console.log(`Button ${foundButton.name} is now ${event.rawValue > 0 ? 'enabled' : 'disabled'}`, event)
    if (!foundButton.onMessages || !foundButton.offMessages) {
      console.log(`Button has no on/off messages, sending CC message to daw output controller ${foundButton.controller} with value ${event.rawValue}`)
      dawMidiOutput.value?.channels[foundButton.channel].sendControlChange(foundButton.controller, event.rawValue)
    }
    if (foundButton.onMessages && foundButton.enabled) {
      console.log('Sending on messages', foundButton.onMessages.map((message) => `CH:${message.channel} CT:${message.controller}`).join(', '))
      foundButton.onMessages.forEach((message) => {
        dawMidiOutput.value?.channels[foundButton.channel].send(getRawMidiMessage(message))
      })
    }
    if (foundButton.offMessages && !foundButton.enabled) {
      console.log('Sending off messages', foundButton.offMessages.map((message) => `CH:${message.channel} CT:${message.controller}`).join(', '))
      foundButton.offMessages.forEach((message) => {
        dawMidiOutput.value?.channels[foundButton.channel].send(getRawMidiMessage(message))
      })
    }
    if (foundButton.sweeps) {
      foundButton.sweeps.forEach((sweep) => {
        activeSweeps.value[sweep.pedal - 1] = foundButton.enabled ? sweep : undefined
      })
    } else {
      activeSweeps.value = []
    }
...
function findBankIndexFromPedal(controller: number, channel: number, buttonTypes: Array<Button['type']> = ['effect', 'trigger', 'program']) {
  return banks.value.findIndex(bank =>
...
function getInfoFromDawByButtonController(controller: number, channel: number, value: number): InfoFromDaw | null {
  ...
  const foundBankByButtonControllerIndex = banks.value.findIndex(bank => {
...
function getInfoFromDawByOnMessages(controller: number, channel: number, value: number): InfoFromDaw | null {
  let foundButtonIndex = -1
  const foundBankByOnMessagesIndex = banks.value.findIndex(bank => {
    const buttonIndex = bank.buttons.findIndex(button =>
...
function getInfoFromDawByOffMessages(controller: number, channel: number, value: number): InfoFromDaw | null {
  let foundButtonIndex = -1
  const foundBankByOffMessagesIndex = banks.value.findIndex(bank => {
    const buttonIndex = bank.buttons.findIndex(button =>
...
onPedalEvent((event) => {
  ...
  if (event.message.type === 'programchange' && foundBankIndex > -1) {
    const button = banks.value[foundBankIndex].buttons
        .find(button =>
  ...
  if (event.message.type === 'controlchange' && foundBankIndex > -1) {
    ...
    const button = banks.value[foundBankIndex].buttons.find(button =>
...
onDawEvent((event) => {
  ...
  if (event.message.type === 'controlchange') {
    ...
    const button = banks.value[bankIndex].buttons[buttonIndex] satisfies Button
    if (button === undefined) {
      throw new Error(`Button not found for channel ${event.message.channel} controller ${event.message.dataBytes[0]}`)
    }
...
<template>
  <EditButtonModal ref="editButtonModal"></EditButtonModal>
  ...
  <MainLayout v-else>
    <template #top-bar>
      <div class="preset-bar grid grid-cols-[40px_1fr_auto]">

        <font-awesome-icon
            :icon="['fa-solid', isEditing ? 'fa-check' : 'fa-edit']" @click="isEditing = !isEditing"
            class="cursor-pointer text-4xl justify-self-center"/>
        <span class="text-4xl justify-self-center">{{ selectedBank.name }}</span>
      </div>
    </template>

    <template #buttons>
      <div class="h-full grid grid-cols-[1fr_auto_auto] ">
        <div class="h-full grid-layout grid grid-cols-5 gap-4 p-4">
          <template
              v-for="(button, index) in selectedBankButtons"
              :key="button.name">
            <PedalButton
                :is-editing="isEditing"
                :style="{ 'order': button.position < 5 ? button.position + 6 : button.position - 6 }"
                :button="button"
        ...
        <div class="h-full w-40 grid-layout grid grid-cols-1 gap-4 p-4">
          <PedalButton
              @click=" handleBankUp() "
          >
            {{ 'Up' }}
